...
class StockOpnameCheck
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if ($request->route('id')) {
            ...
            (int) $countSoHeader = StockOpnameHeader::where('id', $request->route('id'))->where('flag', $statusSoFinished)->count();

            /** If count 0 then abort */
            if ($countSoHeader == 0) {
                return abort(403, 'Unauthorized action. Please select a stock opname in finished state to continue.');
            }
