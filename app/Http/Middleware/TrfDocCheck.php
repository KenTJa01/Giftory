...
class TrfDocCheck
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if ($request->route('id')) {
            ...
            if (!Profile::authorize(InterfaceClass::TRANSFER_PRINT)) {
                return abort(403, 'Unauthorized action. Insufficient permission.');
            }
            ...
            (int) $countTrfHeader = TransferHeader::where('id', $request->route('id'))->where('flag', $statusTrfSubmit)->count();

            /** If count 0 then abort */
            if ($countTrfHeader == 0) {
                return abort(403, 'Unauthorized action. Please select a transfer request in submit state to continue.');
            }
