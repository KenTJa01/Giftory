...
class TrfReqApproveCheck
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if ($request->route('id')) {
            ...
            (int) $countTrfHeader = TransferHeader::where('id', $request->route('id'))->where('flag', $statusTrfPendingApprove)->count();

            /** If count 0 then abort */
            if ($countTrfHeader == 0) {
                return abort(403, 'Unauthorized action. Please select a transfer request in pending approve state to continue.');
            }
