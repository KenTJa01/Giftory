@extends('layouts.main')

@section('containter')

<nav class="navbar navbar-expand-lg navbar-light" id="navbar-partial">
    <div class="container-fluid">

        <button type="button" id="sidebarCollapse" class="btn btn-primary">
            <i class="fa fa-bars"></i>
        </button>

        <h4 style="margin-left: 15px; margin-top: 10px; font-weight: 500; color: #424976">Return</h4>

        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="nav navbar-nav ml-auto">
                <li class="nav-item">
                    <font style="font-weight: 500; font-size: 14px; color: #424976">
                      <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" fill="currentColor" class="bi bi-person-circle" viewBox="0 0 16 16" style="margin-top: -5px; margin-right: 7px">
                          <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"/>
                          <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z"/>
                      </svg>
                      {{ auth()->user()->name }}
                  </font>
                </li>
            </ul>
        </div>
    </div>
</nav>


<div class="row">

    <div class="col text-center" id="title">
        <h2 style="font-weight: 500">Return Approval</h2>
    </div>

    <img src="{{ asset('svg/circleLeft.svg') }}" id="circleLeft" alt="">

    <div class="container text-center mt-3" align="center" style="max-width:100%; margin-top: -10px">
        <img src="{{ asset('svg/circleRight.svg') }}" id="circleRight" alt="">

        <div class="container" id="divTable" style="max-width:100%;">

            <div class="container" id="divTable-white" style="max-width:100%;">


                <div class="row" style="margin: 5px">
                    <div class="col d-flex">
                        <table align="left" style="color: black">
                            <tr>
                                <td align="left" style="font-size: 16px; width: 160px">Return No</td>
                                <td><input type="text" class="form-control inputAdd" name="retNum" id="retNum" value="{{ $ret_header_data?->ret_no }}" style="width: 270px" disabled></td>
                            </tr>
                            <tr>
                                <td align="left" style="font-size: 16px">Return Date</td>
                                <td><input type="text" class="form-control inputAdd" name="retDate" id="retDate" value="{{ date_format(new DateTime($ret_header_data?->ret_date), 'd/m/Y') }}" style="width: 270px" disabled></td>
                            </tr>

                            <tr>
                                <td align="left" style="width: 160px; font-size: 16px">Note</td>
                                <td>
                                    <textarea id="note" class="form-control" maxlength="100" style="height: 100px; margin-top: 1px; max-width: 900px !important;" placeholder="Input note" value="" disabled>{{ $ret_header_data?->note }}</textarea>
                                </td>
                            </tr>
                        </table>
                    </div>
                    <div class="col d-flex">
                        <table align="left" style="color: black">
                            <tr>
                                <td align="left" style="font-size: 16px; width: 160px">Site</td>
                                <td>
                                    <input type="text" class="form-control inputAdd" value="{{ $ret_header_data?->store_code.' - '.$ret_header_data?->site_description }}" style="width: 270px" disabled>
                                </td>
                            </tr>

                            <tr>
                                <td align="left" style="font-size: 16px; width: 160px">Type</td>
                                <td>
                                    <input type="text" class="form-control inputAdd" name="retType" id="retType" value="{{ $ret_type }}" style="width: 270px" disabled>
                                </td>
                            </tr>

                            <tr>
                                @if ( $ret_type == 'Internal' )
                                    <td align="left" style="font-size: 16px">To Location</td>
                                    <td>
                                        <input type="text" class="form-control inputAdd" value="{{ $ret_header_data->location_code.' - '.$ret_header_data->location_name }}" style="width: 270px" disabled>

                                    </td>
                                @else
                                    <td align="left" style="font-size: 16px">To {{ $ret_type }}</td>
                                    <td>
                                        <input type="text" class="form-control inputAdd" value="{{ $ret_header_data->supp_name }}" style="width: 270px" disabled>
                                    </td>
                                @endif
                            </tr>
                            <tr>
                                <td><div style="height: 40px"></div></td>
                            </tr>
                        </table>
                    </div>
                </div>

                <img src="{{ asset('svg/line.svg') }}" alt="" style="width: 100%">

                <h5 style="font-weight: 500">List Item</h5>

                {{-- <div style="height: 290px;" id="tableForm"> --}}
                    <table class="table table-bordered border-dark align-middle" id="table-data" style="width:100%;">
                        <thead class="thead-dark">
                            <tr class="text-center" style="width: 100%; background-color: #35384B; color: white;">
                                <td>Products Category</td>
                                <td>Qty</td>
                                <td>Unit</td>

                                @if ($ret_type == 'supplier')
                                    <td>To location</td>
                                @endif
                            </tr>
                        </thead>
                        <tbody style="background-color: white">
                            @foreach ( $ret_detail_data as $rdd )

                                <tr>
                                    <td>{{ $rdd?->catg_desc.' - '.$rdd?->catg_code }}</td>
                                    @if ($ret_type == 'supplier')
                                        <td>{{ $rdd?->location_name }}</td>
                                    @endif
                                    <td>{{ $rdd?->quantity }}</td>
                                    <td>{{ $rdd?->unit }}</td>

                                </tr>

                            @endforeach
                        </tbody>

                    </table>
                {{-- </div> --}}

            </div>

        </div>

        {{-- CANCELED --}}
        {{-- <button class="btn" id="btn-generate">Generate PDF</button> --}}

        <button type="button" class="btn" id="btn-reject" style="margin-horizontal: 10px;">Reject</button>
        <button type="button" class="btn" id="btn-approve">Approve</button>
    </div>

</div>

<script>
    $.ajaxSetup({
        headers: {
            'X-CSRF-TOKEN' : $('meta[name="csrf-token"]').attr('content')
        }
    });

    $(document).on('click', '#btn-approve', function(event){
        var ret_id = '{{ $ret_header_data?->id }}';
        console.log(ret_id);
        Swal.fire({
            icon: 'warning',
            title: "Are you sure?",
            text: "Approve this transaction",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
            confirmButtonText: "Yes",
            showLoaderOnConfirm: true,
            preConfirm: () => {
                approveRet(event, ret_id)
            }
        });

    });

    function approveRet(event, retId) {
        disableButton();
        $.ajax({
            type: 'POST',
            url: "{{ url('/post-ret-req-approve') }}",
            dataType: 'json',
            data: {
                ret_header_id: retId,
            },
            success: function(response) {
                console.log(response);

                document.getElementById('btn-reject').style.display = 'none';
                document.getElementById('btn-approve').style.display = 'none';
                // document.getElementById('btn-generate').style.display = 'block';

                return Swal.fire({
                    title: response.title,
                    text: response.message,
                    timer: 5000,
                    icon: "success",
                    timerProgressBar: true,
                    showConfirmButton: true,
                    willClose: () => {
                        if (typeof response.route !== "undefined") {
                            window.location.href = response.route;
                        }
                    },
                });
            },
            error: function(error) {
                console.log(error.responseJSON);
                Swal.fire({
                    icon: 'error',
                    title: "Error",
                    text: error.responseJSON.message ?? 'Failed approve return',
                });
                enableButton();
            },
        });
        event.preventDefault();
    }

    $(document).on('click', '#btn-reject', function(event){
        var ret_id = '{{ $ret_header_data?->id }}';

        Swal.fire({
            icon: 'warning',
            title: "Are you sure?",
            text: "Reject this transaction",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
            confirmButtonText: "Yes",
            showLoaderOnConfirm: true,
            preConfirm: () => {
                rejectExp(event, ret_id)
            }
        });

    });

    function rejectExp(event, ret_id){
        disableButton();
        $.ajax({
            type: 'POST',
            url: "{{ url('/post-ret-req-reject') }}",
            dataType: 'json',
            data: {
                ret_header_id: ret_id,
            },
            success: function(response){
                document.getElementById('btn-reject').style.display = 'none';
                document.getElementById('btn-approve').style.display = 'none';

                return Swal.fire({
                    title: response.title,
                    text: response.message,
                    timer: 5000,
                    icon: "success",
                    timerProgressBar: true,
                    showConfirmButton: true,
                    willClose: () => {
                        if (typeof response.route !== "undefined") {
                            window.location.href = response.route;
                        }
                    },
                });
            },
            error: function(error){
                // console.log(error.responseJSON);
                Swal.fire({
                    icon: "error",
                    title: "Error",
                    text: error.responseJSON.message ?? 'Failed reject return',
                });
                enableButton();
            },
        });
        event.preventDefault();
    }

    function enableButton() {
        $("#btn-reject").prop('disabled', false);
        $("#btn-approve").prop('disabled', false);
    }

    function disableButton() {
        $("#btn-reject").prop('disabled', true);
        $("#btn-approve").prop('disabled', true);
    }
</script>

@endsection
